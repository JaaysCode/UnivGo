...
import { FaCalendarAlt, FaClock, FaMapMarkerAlt, FaUsers, FaQrcode } from 'react-icons/fa';
...
export default function Reservation({
  ...
  const getStatusText = () => {
    switch (status) {
      ...
      case 'cancelled_by_admin': return 'Cancelada por admin';
      case 'cancelled_by_user': return 'Cancelada';
              ...
              <div>
                ...
                {guests.length > 0 && (
                  <ul className="mt-1 text-sm text-gray-600">
                    {guests.map(guest => (
                      <li key={guest.id}>• ID: {guest.guestIdentification || guest.id}</li>
                    ))}
                  </ul>
                )}
              </div>
        ...
        {showQR && (
          <div className="mt-3 p-4 border-t border-gray-200">
            <div className="flex flex-col items-center">
              <div className="text-xs text-gray-500 mb-2">Código QR para verificación</div>
